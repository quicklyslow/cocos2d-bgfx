...
+ (NSString*) _generateFreshOpenUDID {
    ...
    /**
     *  BY LXF
     *  用了NSProcessInfo类的- (NSString *)globallyUniqueString方法，返回全球进程的ID。标识包括主机名，进程ID，时间戳，确保ID是唯一的网络。
     *  为了与官方的UDID位数相同，还在MD5值后面追加了8位随机码.
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            openUDID = [OpenUDID _generateFreshOpenUDID];// BY LXF   首次使用UDID，去生成获取。
            
            NSString *string = [NSString stringWithString:@"首次使用UDID，去生成获取"];  
            NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];  
            [ud setObject:string forKey:@"titleKey"]; 
            
        } else {
            // or we leverage the OpenUDID shared by other apps that have already gone through the process
            // 
            openUDID = mostReliableOpenUDID;// BY LXF   已存在UDID，直接去获取。
             
            NSString *string = [NSString stringWithString:@"已存在UDID，直接去获取"];  
            NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];  
            [ud setObject:string forKey:@"titleKey"]; 
     ...
     *  如果用户删除了应用，UserDefaults同样会被清空，为了避免重新生成唯一识别码，该方案还使用到了UIPasteboard类（设备剪切板）。
     *  应用在将唯一识别码保存到UserDefaults的同时，也会将其保存到以特殊的key标识的UIPasteboard中.    
    ...
    **/
         ...
         *  将唯一标识码UDID保存到以特殊的key标识的UIPasteboard中。
...
/**
 *  用到了NSUserDefaults类（应用设置）。应用将获取到的唯一识别码保存到应用的UserDefaults中，如果程序以后需要使用唯一识别码，就从UserDefaults中获取，这样就保证可以拿到同一个识别码。
