...
{
    this->setEventDispatcher(SharedDirector.getEventDispatcher());
    clearEventCallback();
    _armature = nullptr;
    this->release();
}
...
cocos2d::Vec2 DBCCSprite::projectGL(const cocos2d::Vec3& src) const
{
    ...
    cocos2d::Mat4 projMatrix;
    bx::mtxMul(projMatrix, director->getCamera()->getView(), SharedView.getProjection());
...
void DBCCSprite::draw(cocos2d::IRenderer* renderer, const cocos2d::Mat4& transform, uint32_t flags)
{
    ...
    if (_insideBounds)
    {
        ...
        SharedRenderer.push(_polyInfo.triangles.verts, uint32_t(_polyInfo.triangles.vertCount),
