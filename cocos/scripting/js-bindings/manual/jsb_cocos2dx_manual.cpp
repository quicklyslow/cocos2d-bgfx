...
static bool invokeJSKeyboardCallback(EventListenerKeyboard* listener, const char* funcName, KeyCode keyCode, Event* event, se::Value* retVal)
{
    se::ScriptEngine::getInstance()->clearException();
    se::AutoHandleScope hs;
    bool ok = true;
...
static bool js_EventListenerKeyboard_create(se::State& s)
{
    ...
    if (argc == 0)
    {
        ...
        ret->onKeyPressed = [ret](KeyCode keyCode, Event* event) {
            invokeJSKeyboardCallback(ret, "_onKeyPressed", keyCode, event, nullptr);
        };

        ret->onKeyReleased = [ret](KeyCode keyCode, Event* event) {
...
static bool register_eventlistener(se::Object* obj)
{
    ...
    SharedDirector.getEventDispatcher()->setBeforeDispatchEventHook(Event::Type::TOUCH, onBeforeDispatchTouchEvent);
    SharedDirector.getEventDispatcher()->setAfterDispatchEventHook(Event::Type::TOUCH, onAfterDispatchTouchEvent);

    //SharedDirector.getTextureCache()->setTextureCreateHook(onTextureCreate);
    //SharedDirector.getTextureCache()->setTextureDestroyHook(onTextureDestroy);
