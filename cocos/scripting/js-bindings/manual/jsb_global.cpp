        ...
        delegate.onGetStringFromFile = [](const std::string& path) -> std::string{
            ...
            if (FileUtils::getInstance()->isFileExist(byteCodePath)) {
                ...
                if (ZipUtils::isGZipBuffer(data, dataLen))
                {
                    ...
                    ssize_t unpackedLen = ZipUtils::inflateMemory(const_cast<unsigned char*>(data), dataLen, &unpackedData);
                    if (unpackedData == nullptr) {
                        SE_REPORT_ERROR("Can't decrypt code for %s", byteCodePath.c_str());
                        return "";
                    }
            ...
            auto micro = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - startTime);
            std::string jsFilePath = removeFileExt(path) + ".js";
            return FileUtils::getInstance()->getStringFromFile(jsFilePath);
        };
...
bool jsb_enable_debugger(const std::string& debuggerServerAddr, uint32_t port)
{
    ...
    SharedDirector.getScheduler()->scheduleUpdate(&runLoop, 0, false);
    return true;
}
...
{
	//CCLOG("jsb_run_script:%s", filePath.c_str());
...
namespace {
    ...
    static bool require(se::State& s)
    {
        ...
        size_t relativePos = path.rfind('/');
        bool result = false;
        if (relativeDir.length() == 0)
        {
            if (relativePos != std::string::npos && relativePos >= 3)
            {
                relativeDir = path.substr(0, relativePos + 1);
            }
            result = jsb_run_script(path);
        }
        else 
        {
            if (path.find(relativeDir) == std::string::npos)
            {
                result = jsb_run_script(relativeDir + path);
            }
            else
            {
                result = jsb_run_script(path);
...
static bool JSBCore_platform(se::State& s)
{
    ...
    platform = SharedApplication.getTargetPlatform();
...
{
    EventListenerCustom* _event = SharedDirector.getEventDispatcher()->addCustomEventListener(Director::EVENT_AFTER_DRAW, [&](EventCustom *event) {
        SharedDirector.getEventDispatcher()->removeEventListener(_event);
