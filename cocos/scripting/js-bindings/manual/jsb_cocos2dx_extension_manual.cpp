...
static bool js_cocos2dx_extension_loadRemoteImage(se::State& s)
{
    ...
    if (argc == 2)
    {
        ...
        Texture2D* texture = SharedDirector.getTextureCache()->getTextureForKey(url);
        if (texture != nullptr)
        {
            onSuccess(texture);
        }
            ...
            downloader->onDataTaskSuccess = [downloader, url, onSuccess, onError](const cocos2d::network::DownloadTask& task, std::vector<unsigned char>& data){
                ...
                do
                {
                    if (!img->initWithImageData(data.data(), data.size()))
                        break;
                    tex = SharedDirector.getTextureCache()->addImage(img, url);
                } while (0);
                ...
                SharedDirector.getScheduler()->performFunctionInCocosThread([downloader](){
                    delete downloader;
                });
            };
...
static ThreadPool* getThreadPool()
{
    if (_threadPool == nullptr)
    {
        ...
        _resetThreadPoolListener = SharedDirector.getEventDispatcher()->addCustomEventListener(Director::EVENT_RESET, [](cocos2d::EventCustom*){
            CC_SAFE_DELETE(_threadPool);
            SharedDirector.getEventDispatcher()->removeEventListener(_resetThreadPoolListener);
...
static bool js_cocos2dx_extension_initTextureAsync(se::State& s)
{
    ...
    SharedTextureCache.addImageAsyncOnTexture(url, texture, [onCallback](Texture2D* texOutput)
    {
        if (texOutput)
        {
            onCallback(true);
    ...
    //getThreadPool()->pushTask([texture, onCallback, url](int tid){
    ...
    //        //{
    //            SharedDirector.getScheduler()->performFunctionInCocosThread([=]() {
    //                if(SharedTextureCache.addImage(url, data.getBytes(), data.getSize(), texture))
    //                //if (texture->initWithData(data.getBytes(), data.getSize(), Texture2D::PixelFormat::AUTO, 0, 0, Size(0, 0)))
    //                {
    //                    onCallback(true);
    //                }
    //                else
