...
void onHelpAbout()
{
    DialogBox(GetModuleHandle(NULL),
        ...
        SharedDirector.getOpenGLView()->getWin32Window(),
              AboutDialogCallback);
}

void shutDownApp()
{
    auto glview = dynamic_cast<GLViewImpl*> (SharedDirector.getOpenGLView());
...
int SimulatorWin::run()
{
    ...
    SharedApplication.setAppDelegate(_app);
...
void SimulatorWin::setupUI()
{
    ...
    // show FPs
    bool displayStats = SharedDirector.isDisplayStats();
...
{
    _project.setFrameScale(frameScale);
    SharedDirector.getOpenGLView()->setFrameZoomFactor(frameScale);
}
